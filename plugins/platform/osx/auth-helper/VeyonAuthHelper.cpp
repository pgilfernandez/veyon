...
{
	auto reply = static_cast<pam_response *>( std::malloc( sizeof(struct pam_response) * static_cast<size_t>( num_msg ) ) );
	if( reply == nullptr )
	{
		return PAM_CONV_ERR;
	}
...
int main()
{
	...
	QString password;
	QFile stdIn;
	stdIn.open( stdin, QFile::ReadOnly );
